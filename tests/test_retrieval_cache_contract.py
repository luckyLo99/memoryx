...
def test_no_lru_cache() -> None:
    repo_path = Path(__file__).parent.parent / "memoryx" / "storage" / "repository.py"
    text = repo_path.read_text()
    assert "lru_cache" not in text
    # Check retrieval engine too
    engine_path = Path(__file__).parent.parent / "memoryx" / "retrieval" / "engine.py"
...
def test_no_context_cache() -> None:
    ...
    ctx_path = Path(__file__).parent.parent / "memoryx" / "context" / "engine.py"
