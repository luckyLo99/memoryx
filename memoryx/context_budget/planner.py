...
from .profiles import ContextMode, ContextProfile, clamp_profile_to_model_window, get_context_profile
...
class AdaptiveContextPlanner:
    ...
    def plan(self, query: str, requested_mode: str | None = None) -> ContextPlan:
        q = query or ""
        if requested_mode:
            selected = requested_mode.lower()
            reason = f"explicit mode={selected}"
        elif matches_any(q, DEBUG_PATTERNS):
            selected = "debug"
            reason = "matched debug intent"
        elif matches_any(q, DEEP_PATTERNS):
            selected = "deep"
            reason = "matched deep technical intent"
        elif matches_any(q, BRIEF_PATTERNS) or len(q) < 30:
            selected = "brief"
            reason = "matched brief intent"
        else:
            selected = "standard"
