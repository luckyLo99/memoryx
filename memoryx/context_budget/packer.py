...
class ContextPacker:
    def __init__(self, policy: ContextBudgetPolicy | None = None, estimator: TokenEstimator | None = None):
        self.policy = policy or ContextBudgetPolicy.from_env()
    ...
    def pack(self, *, request_id: str, session_id: str | None, query: str, items: list[ContextItem],
        ...
        max_tokens = self.policy.memory_budget_tokens()
        ...
        sections["task_focus"].append({"content": task_focus, "estimated_tokens": task_tokens})
        ...
        for item in sorted(items, key=lambda x: (-x.score, x.item_id)):
            if omit_repeated_items and item.item_id in previous_item_ids and item.section == "relevant_memories":
                repeated += 1
                omitted_repeated.append(item.item_id)
                continue
            if item.score < self.policy.min_score and item.section == "relevant_memories":
                dropped += 1
                continue
            if item.section == "relevant_memories" and sum(1 for x in sections["relevant_memories"]) >= self.policy.max_memory_items:
                dropped += 1
                continue
            if item.section in {"session_context", "session_summary"} and sum(1 for x in sections.get(item.section, [])) >= self.policy.max_session_items:
                dropped += 1
                continue
            ...
            section_used[item.section] = used + tc
            remaining -= tc
            included += 1

        used = max_tokens - remaining
        if dropped:
            warnings.append(f"dropped {dropped} items due to budget/scope/score limits")
        if repeated:
            warnings.append(f"omitted {repeated} repeated items from previous pack")
        ...
        if sections.get("relevant_memories"):
            ...
            lines.append("- No relevant memories selected within budget.")
