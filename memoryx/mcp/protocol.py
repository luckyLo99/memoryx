...
class MCPToolRegistry:
    ...
    def call(self, name: str, arguments: dict[str, Any] | None = None) -> ToolCallResult:
        if name not in self.specs:
            return ToolCallResult(ok=False, error=f"unknown tool: {name}", tool_name=name)
        args = arguments or {}
        try:
            result = self.handlers[name](args)
            return ToolCallResult(ok=True, data=result, tool_name=name)
        except Exception as exc:
            return ToolCallResult(ok=False, error=str(exc), tool_name=name)